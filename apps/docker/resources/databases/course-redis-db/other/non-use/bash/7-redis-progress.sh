...
psql -h "$PGHOST" -U "$PGUSER" -d "$PGDATABASE" -c "\copy (SELECT id, lesson_id, user_id, completed, completed_at FROM progress) TO STDOUT WITH CSV" > "$CSV_FILE"
if [ $? -ne 0 ]; then
...
awk -F',' '{ printf("HSET progress:%s lesson_id \"%s\" user_id \"%s\" completed \"%s\" completed_at \"%s\"\r\n", $1, $2, $3, $4, $5) }' "$CSV_FILE" > "$REDIS_FILE"
if [ $? -ne 0 ]; then
